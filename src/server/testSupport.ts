...
/**
 * Seeds a throwaway working DB (from the synthetic sample backup fixture) in
 * a temp directory, and resets vitest's module registry so `db.server.ts`'s
...
export async function setupTestWorkingDb(): Promise<TestDbContext> {
  ...
  const sampleBackup = buildSampleBackupBytes()
  fs.writeFileSync(dbModule.WORKING_DB_PATH, sampleBackup)
  fs.writeFileSync(dbModule.ORIGINAL_IMPORT_DB_PATH, sampleBackup)

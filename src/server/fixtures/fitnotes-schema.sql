-- Structure-only schema for the FitNotes Android export format.
--
-- Extracted with `sqlite3 <a real FitNotes backup> .schema` on 2026-07-06.
-- Contains no personal data — CREATE TABLE statements only. This is what
-- sampleBackup.ts executes to build a synthetic fixture DB with the same
-- shape as a real export, so tests never need a real user's backup file
-- (which can't be committed — see .gitignore's `*.fitnotes` rule).
--
-- Re-extract by pointing sqlite3 at a fresh backup if the app schema
-- version (PRAGMA user_version, see EXPECTED_USER_VERSION in db.server.ts)
-- ever changes.
...
CREATE TABLE Goal (_id INTEGER PRIMARY KEY AUTOINCREMENT, type_id INTEGER NOT NULL, exercise_id INTEGER NOT NULL, metric_weight INTEGER NOT NULL, reps INTEGER NOT NULL, unit INTEGER NOT NULL, title TEXT, target_date TEXT, sort_order INTEGER NOT NULL DEFAULT 0, distance INTEGER NOT NULL DEFAULT 0, duration_seconds INTEGER NOT NULL DEFAULT 0, start_date TEXT);

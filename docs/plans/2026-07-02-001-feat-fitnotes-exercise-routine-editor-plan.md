...
- **KTD11 — Shared `Modal` primitive (`src/components/Modal.tsx`) replaces full-page/inline-input flows for "add" actions.** A plain controlled React component (`{ open, onClose, title, children }`, no new dependency) — closes on backdrop click, Escape, or its `×` button, but not on clicks inside the panel. Added once enough "add" flows (exercise, category, routine-section-exercise) needed the same pop-up treatment that inline `<select>`s and full routes were making cumbersome. **Editing an exercise still uses its own route** (`exercises/$exerciseId.tsx`) — only exercise/category _creation_ and adding an exercise to a routine section moved into modals; the create form itself was extracted into `src/components/ExerciseForm.tsx` so the modal and the edit route share one implementation (`{ exercise?, categories, defaultCategoryId?, onSaved, onCancel }`).
- **KTD12 — A category is required on every exercise; no silent default.** Originally new exercises defaulted `categoryId` to `categories[0]?.id` (i.e. always "Abs" unless changed). That default is gone: the create form starts with `categoryId` unset and a disabled "Select a category…" placeholder, Save is blocked client-side until one is chosen, and `createExercise`/`updateExercise` (`exercises.server.ts`) throw `Category is required.` / `Category not found.` server-side via a shared `assertValidCategory` check — the real gate, since the client check alone isn't trusted at the server boundary. `ExerciseInput.categoryId` is typed `number | null | undefined` to let the unset state cross that boundary instead of coercing it to a fake value earlier.
- **KTD13 — Move-unused-exercises alert shows the full candidate list behind a "Show details" toggle, not just a count.** `listUnusedExercises()` (`exercises.server.ts`) exports the same underlying candidates the count was already quietly computing (`listUnusedCandidates` + `findUnusedCategoryId`), so the UI can show exactly which exercises would move and their resulting prefixed name (e.g. `Abs - Incline Crunch`) before the user confirms — nothing new is computed, just surfaced.
...
- **Approach:** Server functions cover routine, `RoutineSection`, `RoutineSectionExercise`, and `RoutineSectionExerciseSet` CRUD. Reordering within a section (KTD6) persists via each table's `sort_order` column. Removing an exercise from a section (which must also remove its target sets) and any batch reorder both run as a single transaction (KTD8), so a mid-operation failure can't leave orphaned set rows or a partially-renumbered `sort_order`. Deleting a routine section applies KTD4's reference guard against `WorkoutGroup.routine_section_id` and `WorkoutGroupExercise.routine_section_id` before deleting. Adding an exercise to a section is a `Modal` listing that section's available exercises rather than the original inline `<select>` + button (KTD11). The routine detail page has a `← Back` link to `/routines` above its header.
- **Test scenarios:**
